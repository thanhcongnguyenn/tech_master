...
const BrandManager = () => {
    ...
    const fetchCategoriesWithParams = async (params) => {
        try {
            const response = await apiBrandService.getLists(params);
    ...
    const handleAddEditCategory = async (values) => {
        ...
        try {
            ...
            if (editingCategory) {
                const response = await apiBrandService.update(editingCategory.id, {...editingCategory, ...values});
                const params = Object.fromEntries([...searchParams]);
                fetchCategoriesWithParams({ ...params, page: params.page || 1, page_size: params.page_size || 10 });
            } else {
                const response = await apiBrandService.add(values);
                const params = Object.fromEntries([...searchParams]);
                fetchCategoriesWithParams({ ...params, page: params.page || 1, page_size: params.page_size || 10 });
            }
    ...
    const handleDeleteData = async () => {
        try {
            await apiBrandService.delete(categoryToDelete.id);
            ...
            <Row className="gutters mt-3">
                <Col xl={12}>
                    <Breadcrumb>
                        ...
                        <Nav.Item>
                            <Nav.Link as={Link} to="/admin/ecommerce/brand">Thương hiệu</Nav.Link>
                        </Nav.Item>
